...
class S3Dump(CLI):
    def __init__(self, args):
        super(S3Dump, self).__init__(args)

        self.mutex_directory = Lock()
        self.mutex_interesting = Lock()
    ...
    def run(self):
        super(S3Dump, self).run()
        display.banner("S3Dump")
