...
from s3dump import context, constants as C
from s3dump.utils._text import to_bytes, to_text
from s3dump.utils.six import with_metaclass
from s3dump.utils.color import stringc
from s3dump.utils.singleton import Singleton
...
class Display(with_metaclass(Singleton, object)):
    ...
    def display(self, msg, color=None, stderr=False, screen_only=False, log_only=False, newline=True):
        ...
        if not log_only:
            ...
            if sys.version_info >= (3,):
                msg2 = to_text(
                    msg2, self._output_encoding(stderr=stderr), errors='replace')
    ...
    def warning(self, msg):
        self.display(u"[WARNING]: %s" % msg, color=C.COLOR_WARN, stderr=True)
